...
class Quiz(models.Model):
    ...
    random_order = models.BooleanField(
        ...
        verbose_name=_("Перемешать вопросы"))

    max_questions = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Количество вопросов"),
        help_text=_("Количество вопросов, на которые ужно ответить при каждой попытке."))

    answers_at_end = models.BooleanField(
        blank=False, default=False,
        help_text=_("Правильный ответ не отображается после вопроса."
                    " Ответы отображаются в конце."),
        verbose_name=_("Ответы отображаются в конце"))

    exam_paper = models.BooleanField(
        blank=False, default=False,
        help_text=_("Если да, то результат каждой попытки"
                    " пользователя будет сохранен. Необходимо для маркировки."),
    ...
    pass_mark = models.SmallIntegerField(
        ...
        verbose_name=_("Минимальный процент"),
        help_text=_("Процент для минимального прохождения теста."),
    ...
    draft = models.BooleanField(
        ...
        help_text=_("Если да, то тест не отображается"
                    "в списке викторин и может быть только"
                    "взято пользователями, которые могут редактировать тест."))
...
class Progress(models.Model):
    ...
    correct_answer = models.CharField(max_length=10, verbose_name=_('Правельные ответы'))

    wrong_answer = models.CharField(max_length=10, verbose_name=_('Неправельные ответы'))
    ...
    def __str__(self):
        return self.user.name + ' ' + self.user.last_name + ' - '  + self.score
